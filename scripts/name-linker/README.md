...
Scripture and 6,214 across the app's own prose**. Until this directory existed it had no tests at
...
| `prose-links.tsv` | 6,214 | every person-link in every authored prose block the app puts through `LinkedVerseText` |
| `key-totals.tsv` | 3,987 | a tally covering **every** kind — location, POI, topic, timeline, verse reference — one row per (kind, matched text, id, path) |
...
show. Its 3,987 rows currently break down as 2,304 verse references, 733 person, 386 location, 354
...
| `topics.ts` — section paragraphs | 1,028 |
...
differently between the two paths**, and all but a handful of the 6,214 prose links run with no
...
- **Five fields that are links on the PUBLIC pages and plain text in the app.** `scripts/seo/render.mjs`
  ...
  `location.rulers[].name` through the same linker when it generates the ~970 pre-rendered pages.
  The app renders all five as plain text, so `loadProseBlocks()` correctly does not enumerate them
  and none of the three snapshots covers a single one — 949 blocks that a stranger can read on
