...
Scripture and 6,286 across the app's own prose**. Until this directory existed it had no tests at
...
| `prose-links.tsv` | 6,286 | every person-link in every authored prose block the app puts through `LinkedVerseText` |
...
| `topics.ts` — section paragraphs | 1,190 |
...
differently between the two paths**, and all but a handful of the 6,286 prose links run with no
