...
Scripture and 6,176 across the app's own prose**. Until this directory existed it had no tests at
...
| `prose-links.tsv` | 6,176 | every person-link in every authored prose block the app puts through `LinkedVerseText` |
| `key-totals.tsv` | 3,968 | a tally covering **every** kind — location, POI, topic, timeline, verse reference — one row per (kind, matched text, id, path) |
...
show. Its 3,968 rows currently break down as 2,296 verse references, 732 person, 386 location, 344
...
| `topics.ts` — section paragraphs | 962 |
...
differently between the two paths**, and all but a handful of the 6,176 prose links run with no
