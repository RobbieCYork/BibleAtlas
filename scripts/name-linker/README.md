...
Scripture and 5,689 across the app's own prose**. Until this directory existed it had no tests at
...
at all** — every case had to be a verse, so the only cover the 5,689 prose links had was the
...
| `prose-links.tsv` | 5,689 | every person-link in every authored prose block the app puts through `LinkedVerseText` |
| `key-totals.tsv` | 3,693 | a tally covering **every** kind — location, POI, topic, timeline, verse reference — one row per (kind, matched text, id, path) |
...
show. Its 3,693 rows currently break down as 2,070 verse references, 727 person, 379 location, 308
...
| `topics.ts` — section paragraphs | 598 |
...
differently between the two paths**, and all but a handful of the 5,689 prose links run with no
